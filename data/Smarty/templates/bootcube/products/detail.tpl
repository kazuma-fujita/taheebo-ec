		...
		<div class="row">
			...
			<div id="detailphotobloc" class="col-sm-5 col-lg-4">
				...
				<div class="photo">
					...
					<!--{/if}-->
						<img src="<!--{$smarty.const.IMAGE_SAVE_URLPATH}--><!--{$arrProduct.main_large_image|h}-->" width="<!--{$arrFile[$key].width}-->" height="<!--{$arrFile[$key].height}-->" alt="<!--{$arrProduct.name|h}-->" class="picture" />
					<!--{if $arrProduct.main_image|strlen >= 1}-->
						</a>
					<!--{/if}-->
				</div>
			...
			<div id="detailrightbloc" class="col-sm-7 col-lg-8">
				...
				<div class="cart_area">
					...
					<!--{if $tpl_stock_find}-->
						...
						<div class="cartin">
							<div class="quantity">
								数量：<input type="text" class="box60 form-control" name="quantity" value="<!--{$arrForm.quantity.value|default:1|h}-->" maxlength="<!--{$smarty.const.INT_LEN}-->" style="<!--{$arrErr.quantity|sfGetErrorColor}-->" />
								<!--{if $arrErr.quantity != ""}-->
					...
					<div class="item_panel col-xs-6 col-sm-3">
						<div class="item_image">
							<a href="<!--{$smarty.const.P_DETAIL_URLPATH}--><!--{$arrItem.product_id|u}-->">
								<img src="<!--{$smarty.const.IMAGE_SAVE_URLPATH}--><!--{$arrItem.main_large_image|sfNoImageMainList|h}-->" alt="<!--{$arrItem.name|h}-->" class="img-responsive" />
							</a>
						</div>
						...
						<div class="item_meta">
							<p class="title">
								<a href="<!--{$smarty.const.P_DETAIL_URLPATH}--><!--{$arrItem.product_id|u}-->"><!--{$arrItem.name|h}--></a>
							</p>
							<p class="sale_price">
							...
							<p class="mini comment"><!--{$arrItem.comment|h|nl2br}--></p>
						</div>
					</div>
					<!--{if $smarty.foreach.arrRecommend.iteration % 4 === 0 and $smarty.foreach.arrRecommend.last == false}-->
						</div>
						<div class="row">
					<!--{/if}-->
				<!--{/foreach}-->
