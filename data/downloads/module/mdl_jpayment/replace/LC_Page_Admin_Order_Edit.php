 ...
 * @version $Id: LC_Page_Admin_Order_Edit.php 23251 2013-10-24 10:49:16Z undertree $
 */
    ...
    public function action()
    {
        ...
        switch ($this->getMode()) {
            ...
            case 'edit':
                ...
                /// modified start
                $arrInputFormList = $objFormParam->getDbArray();
                // フォームから「対応状況」を取得
                $strModifiedOrderStatus = $arrInputFormList['status'];
                /// modified end
                ...
                if (SC_Utils_Ex::isBlank($this->arrErr)) {
                    ...
                    /// modified start
                    if ($order_id >= 0) {
                        if (file_exists(MODULE_REALDIR . 'mdl_jpayment/module/Gateway.php')) {
                            require_once(MODULE_REALDIR . 'mdl_jpayment/module/Gateway.php');
                            $objGateway = new Gateway();
                            switch ($strModifiedOrderStatus) {
                                case ORDER_PRE_END:
                                    $ret = $objGateway->sfPostSalesData($order_id);
                                    break;
                                case ORDER_CANCEL:
                                    $ret = $objGateway->sfPostCancelData($order_id);
                                    break;
                            }
                        }
                    }
