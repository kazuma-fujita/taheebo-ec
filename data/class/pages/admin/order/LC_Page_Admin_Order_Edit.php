 ...
 * @version $Id: LC_Page_Admin_Order_Edit.php 23394 2014-05-03 06:19:51Z kimoto $
 */
class LC_Page_Admin_Order_Edit extends LC_Page_Admin_Order_Ex
{
    ...
    public function action()
    {
        ...
        if (!SC_Utils_Ex::isBlank($order_id)) {
            ...
            // add
            $arrValuesBefore['status'] = $objFormParam->getValue('status');
        } else {
            $this->tpl_subno = 'add';
            $this->tpl_mode = 'add';
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        // add 
        $objFormParam->addParam("メールを自動送信しない", "disable_auto_mail", INT_LEN, 'n', array("SPTAB_CHECK", "NUM_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam('伝票番号', 'deliv_number', MTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    }
    ...
    public function doRegister($order_id, &$objPurchase, &$objFormParam, &$message, &$arrValuesBefore)
    {
        ...
        // add
        if ($order_id > 0) {
            if ($arrValues['disable_auto_mail'] != 1) {
                if ($arrValues['status'] != $arrValuesBefore['status']) {
                    if (ORDER_DELIV == $arrValues['status']) {
                        $this->doSend($objFormParam, 9); // 発送完了メール
                    }
                    if (ORDER_CANCEL == $arrValues['status']) {
                        $this->doSend($objFormParam, 3); // キャンセルメール
                    }
                }
            } else {
                $objFormParam->setValue('disable_auto_mail', '');
            }
        }
        //////////
        return $order_id;
